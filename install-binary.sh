...
[ -z "$HELM_PLUGIN_NAME" ] && HELM_PLUGIN_NAME="helm-datarobot"
[ -z "$HELM_BIN" ] && HELM_BIN=$(command -v helm)
[ -z "$HELM_PLUGINS" ] && HELM_PLUGINS=$(helm env | grep 'HELM_PLUGINS' | cut -d '=' -f2 | tr -d '"')
[ -z "$HELM_PLUGIN_DIR" ] && HELM_PLUGIN_DIR="$HELM_HOME/plugins/$HELM_PLUGIN_NAME"
